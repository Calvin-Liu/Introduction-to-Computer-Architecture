...
//	ringbuf = malloc(ksize * sizeof(float));
	...
	for (i = 0; i < ksize; i++)
		kernel[i] *= scale;
#pragma omp parallel private(x,y,ringbuf) //6
{
	//ringbuf = malloc(ksize * sizeof(float)); //7
	// blur each row
	#pragma omp for //8
	for (y = 0; y < h; y++)
	{
...
#pragma omp parallel private(x,y)
{
	...
	for (x = 0; x < w; x++)
	{
		...
		for (j = 0; j < 3; j++)
		{
			int r = y+i-1; if(r<0)r=0; else if(r>=h)r=h-1;
			int c = x+j-1; if(c<0)c=0; else if(c>=w)c=w-1;
...
#pragma omp parallel private(x) //1
{
	#pragma omp for //2
	for (y = 0; y < h; y++)
	for (x = 0; x < w; x++)
