...
main(int argc, char *argv[])
{
	...
	const int BLKDIM = 4;
	int nblkx = DIVCEIL(W,BLKDIM);
	int nblky = DIVCEIL(H,BLKDIM);
	...
	juliaKernel<<<gridDim,blockDim>>>(img_d, W, H, scale);
	checkCUDAerror("juliaKernel");

	// Step 4 of 4: Copy image from GPU
	cudaMemcpy(img, img_d, nbytes, cudaMemcpyDeviceToHost);
